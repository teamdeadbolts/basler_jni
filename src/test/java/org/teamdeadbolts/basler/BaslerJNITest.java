...
/**
 * JUnit test for BaslerJNI
 * Note: These tests will be skipped if no physical cameras are connected
...
public class BaslerJNITest {
    ...
    void testExposure() {
        ...
        try {
            ...
            assertEquals(exposureValue, retrievedExposure, 100.0, "Exposure should match within tolerance");
        } finally {
            BaslerJNI.destroyCamera(handle);
        }
    }
    ...
    void testFrameRate() {
        ...
        try {
            ...
            assertEquals(frameRate, retrievedFrameRate, 5.0, "Frame rate should match within tolerance");
        } finally {
            BaslerJNI.destroyCamera(handle);
        }
    }
    ...
    void testInvalidHandle() {
        ...
        assertEquals(-1.0, BaslerJNI.getExposure(invalidHandle), "Should return -1 for invalid handle");
    ...
    static void tearDown() {
        ...
        if (!libraryLoaded) {
            System.out.println("Note: Most tests were skipped due to missing native library");
        } else if (!hasCameras) {
            System.out.println("Note: Camera-dependent tests were skipped (no cameras connected)");
        }
    }
}